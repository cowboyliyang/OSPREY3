#!/bin/bash
# Round 2: S8 with dual_enc, multihead, transfer
# Run after round 1 finishes
...
for ARCH in dual_enc multihead transfer; do
    SUFFIX="model_subtree_${ARCH}"
    PSUBTREE="$BASE/protein/$SUFFIX/subtree_model.onnx"
    CSUBTREE="$BASE/complex/$SUFFIX/subtree_model.onnx"

    sbatch $SLURM --job-name=bench_s8_${ARCH} \
