...
public class ProposalRootTestRunner {
    public static void main(String[] args) {
        var request = LauncherDiscoveryRequestBuilder.request()
            .selectors(selectClass("edu.duke.cs.osprey.packstar.TestPackStarForwardTripleSelection"),
                       ...
                       selectClass("edu.duke.cs.osprey.branchdp.TestHigherOrderFullDP"))
            .build();
        ...
        if (listener.getSummary().getTestsFoundCount() < 60
                || listener.getSummary().getTestsFailedCount() != 0)
            throw new IllegalStateException("proposal-root regression tests failed");
    }
}
