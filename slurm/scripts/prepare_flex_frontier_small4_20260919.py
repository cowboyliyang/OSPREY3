...
SYSTEMS = {'3bu8': [7,8,9,10], '4wem': [6,7,8,9], '2rf9': [7,8,9,10], '5it3': [7,8,9,10]}
...
for system,levels in SYSTEMS.items():
    ...
    for k in levels:
        ...
        row=dict(task_id=len(designs),design_id=did,system=system,flex_delta=k,
                 ...
                 expected_sequences=21 if system == '4wem' else 1+19*len(mutable),
        ...
        designs.append(row)
assert len(designs)==16 and len(SYSTEMS)==4
...
protocol={
    ...
    'time_limit_days':14,'account':'grisman','partition':'grisman,compsci','exclude_nodes':'fennario-[01-06]',
    ...
    'selection':'Four smallest remaining baseline systems supplement the existing 16 systems / 71 designs; no existing design is replaced',
    'level_choice':'3bu8,2rf9,5it3 +7..+10; 4wem +6..+9. Four consecutive levels starting above one day under historical-time times 3**k heuristic. Upper levels may exceed 14 days and are censored. Frozen geometric ordering; no measured speedup assumed.',
    'sequence_exception':'4wem B347 WT is HID, distinct from allowed HIS: WT plus 20 substitutions gives 21 sequences. Preserve WT and full allowed alphabet.',
    ...
    'packstar_status':'Additional H200 cohort only; preserve the running 16-system build, inputs, indices and outputs',
}
(package/'protocol.json').write_text(json.dumps(protocol,indent=2)+'\n')
lines=['# Four additions: 16 designs, combined cohort 20 systems / 87 designs','',
...
lines += ['', 'All 16 actual OSPREY position/sequence preflights passed. Fixed mutable residues and sequence order per system.',
          '16 CPU, FP64, CPU CCD, epsilon=0.683, stability filter off, fresh EMAT, 14-day limit.',
          'Original 16-system H200 experiments stay intact. Use a separate additional input package and result root.']
(package/'README.md').write_text('\n'.join(lines)+'\n')
checks=[]
for path in sorted(package.rglob('*')):
    if path.is_file(): checks.append(hashlib.sha256(path.read_bytes()).hexdigest()+'  '+str(path.relative_to(package)))
(package/'SHA256SUMS').write_text('\n'.join(checks)+'\n')
(ROOT/'READY').write_text('16 designs validated\n')
