"""Freeze and validate the five additions to the twelve-system MARK* cohort."""
import csv
import hashlib
import io
import json
...
estimate={'pdb_files':len(inputs),'pdb_bytes':sum(p.stat().st_size for p in inputs),
          ...
          'notes':'Five PDBs, small manifests and one Java overlay. No dataset download. Long-run caches/log sizes depend on search; all stay in xtmp.'}
...
for system,levels in SYSTEMS.items():
    ...
    for k in levels:
        ...
        designs.append(row)
assert len(designs)==26 and len(SYSTEMS)==5
...
protocol={
    ...
    'memory_gib':192,'heap_gib':160,'time_limit_days':14,'account':'grisman','partition_preference':'compsci or non-A5000 grisman, all designs eligible together',
    ...
    'selection':'Five new systems added to seven retained systems; the combined twelve-system cohort is performance-selected, not an unbiased 38-system aggregate',
    'retained_markstar_array':'12631847',
    'retained_task_ids':list(range(16))+list(range(28,32)),
    'cancelled_task_ids':list(range(16,28)),
    'level_choice':'Fixed before submission; inherited frozen geometry order. For finite short baselines, top k follows nearest integer log_3(14 days / historical time); include every intermediate k. 1a0r uses -2,-1,0 because baseline timed out at 14 days.',
    'historical_1a0r_warning':'Old MARK* job 11899109 recorded bounds warnings and timed out; included by explicit request, without assuming the warning caused timeout',
...
lines=['# Five additions to the twelve-system MARK* flexibility frontier','',
       '26 independent new designs plus 20 retained designs. No positive flex expansion for systems whose historical MARK* baseline exceeds seven days.',
...
lines += ['', 'Execution: account=grisman, prefer partition=compsci, overflow to non-A5000 grisman; 16 CPUs, 192 GiB memory, 160 GiB Java heap, 14 days, no GPU.',
          ...
          'All 26 new OSPREY conformation-space preflights passed; positions and sequence counts/order were checked.',
...
(ROOT/'READY').write_text('26 designs validated\n')
