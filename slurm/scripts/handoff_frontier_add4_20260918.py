...
assert (prep / 'READY').read_text().strip() == '25 designs validated'
source = prep / 'package'
...
for design in designs:
    ...
    rows.append(row)
assert len(rows) == 25 and len({r['design_id'] for r in rows}) == 25
...
for name in ('run_add4.slurm',):
    subprocess.run(['bash', '-n', str(repo / 'slurm/h200' / name)], check=True)
for name in ('prepare_flex_frontier_add4_20260918.slurm', 'run_markstar_flex_add4_20260918.slurm'):
...
summary = dict(designs=25, systems=4, sequences=sum(int(r['expected_sequences']) for r in rows),
               ...
               validation='25 added and 1 original real sequence_dump runs via portable CPU preflight; no H200 GPU execution',
               markstar_arrays=[small_job, large_job])
(root / 'audit.json').write_text(json.dumps(summary, indent=2) + '\n')
(root / 'READY').write_text('25 additional designs ready for transfer; original cohort retained\n')
