...
public class MARKStarBound implements PartitionFunction.WithConfDB {
    ...
    protected void processFullConfNode(List<MARKStarNode> newNodes, MARKStarNode curNode, Node node) {
        ...
        // 在minimize之前记录bounds
        final BigDecimal preMinimizeSubtreeUpper = node.getSubtreeUpperBound();
        final BigDecimal preMinimizeSubtreeLower = node.getSubtreeLowerBound();
        final BigDecimal preMinimizeErrorBound = curNode.getErrorBound();
        ...
        loopTasks.submit(() -> {
            try (ObjectPool.Checkout<ScoreContext> checkout = contexts.autoCheckout()) {
                ...
                synchronized(this) {
                    ...
                    printMinimizationOutput(node, newConfLower, oldgscore, oldConfLower, oldConfUpper,
                        preMinimizeSubtreeUpper, preMinimizeSubtreeLower, preMinimizeErrorBound);
                }
    ...
    private void printMinimizationOutput(Node node, double newConfLower, double oldgscore) {
        printMinimizationOutput(node, newConfLower, oldgscore, Double.NaN, Double.NaN,
            BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO);
    }

    private void printMinimizationOutput(Node node, double newConfLower, double oldgscore, double oldConfLower, double oldConfUpper,
                                        BigDecimal preSubtreeUpper, BigDecimal preSubtreeLower, BigDecimal preErrorBound) {
        if (printMinimizedConfs) {
            System.out.println("[" + SimpleConfSpace.formatConfRCs(node.assignments) + "]"
                    + String.format("conf:%4d, score:%12.6f, lower:%12.6f, corrected:%12.6f energy:%12.6f"
                            +", confBounds:[%12.6f, %12.6f], subtreeBounds:[%12e, %12e], errorBound:%12e"
                            +", globalBounds:[%12e, %12e], delta:%12.6f, time:%10s",
                    numConfsEnergied, oldgscore, minimizingEmat.confE(node.assignments),
                    correctionMatrix.confE(node.assignments), newConfLower,
                    oldConfLower, oldConfUpper,
                    preSubtreeLower.doubleValue(), preSubtreeUpper.doubleValue(), preErrorBound.doubleValue(),
