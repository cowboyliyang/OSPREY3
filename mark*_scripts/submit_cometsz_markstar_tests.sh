...
#SBATCH --job-name=cometsz-markstar
#SBATCH --output=cometsz_markstar_%j.out
#SBATCH --error=cometsz_markstar_%j.err
...
# Run COMETS-Z with MARK* performance tests
echo ""
echo "=========================================="
echo "Running COMETS-Z Performance Tests"
echo "GradientDescentPfunc vs MARK* (MARKStarBound)"
echo "=========================================="
echo "Test suite: TestCometsZWithMARKStarPerformance"
...
if [ $TEST_EXIT_CODE -eq 0 ]; then
  ...
  echo "  grep 'MARK\* Speedup:' cometsz_markstar_${SLURM_JOB_ID}.out"
  echo "  grep 'Total Time:' cometsz_markstar_${SLURM_JOB_ID}.out"
  echo "  grep 'Execution Time:' cometsz_markstar_${SLURM_JOB_ID}.out"
else
  echo ""
  echo "✗ Tests FAILED with exit code $TEST_EXIT_CODE"
  echo "Check error file: cometsz_markstar_${SLURM_JOB_ID}.err"
fi
