...
def combine_Confspaces(tfile: str, pfile: str):
    ...
    dholder = tfile.split("[")[1]
    doublet = dholder.split("]")[0]
    to_trim = ("target-[%s].confspace" % doublet)
    destination = tfile.replace(to_trim, "") + "complex-[" + doublet + "].confspace"
