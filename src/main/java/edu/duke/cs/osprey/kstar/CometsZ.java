...
public class CometsZ {
	...
	public static class State {
		...
		/**
		 * set this to a File if you want to use ConfDB with COMETS-Z
		 */
		...
		public ConfDBs() {
			...
			for (State state : states) {
				...
				if (confdb != null) {
					...
					tables.put(state, confdb.new ConfTable("COMETS-Z"));
				}
			}
		}
	}
	...
	public class SequenceInfo {
		...
		public SequenceInfo(SeqAStarNode node, SeqConfs confs) {
			...
			objective = CometsZ.this.objective.calc(confs);
			for (LMFE constraint : CometsZ.this.constraints) {
	...
	public static class Builder {
		...
		/**
		 * The energy window is actually necessary for COMETS-Z to finish in a reasonable
		 ...
		 * COMETS-Z would enumerate every sequence while trying to find the desired number of
		 * sequences. This would effectively make COMETS-Z run in time that is linear in the
		...
		public CometsZ build() {
		    CometsZ mskStar = new CometsZ(objective, constraints, epsilon, objectiveWindowSize, objectiveWindowMax, maxSimultaneousMutations, minNumConfTrees, printToConsole, logFile);
	...
	private CometsZ(LMFE objective, List<LMFE> constraints, double epsilon, double objectiveWindowSize, double objectiveWindowMax, int maxSimultaneousMutations, Integer minNumConfTrees, boolean printToConsole, File logFile) {
		...
		if (states.isEmpty()) {
			throw new IllegalArgumentException("COMETS-Z found no states");
		}
	...
	public List<SequenceInfo> findBestSequences(int numSequences) {
		...
		try (ConfDBs confDBs = new ConfDBs()) {
			...
			log("\nCOMETS-Z searching for the %d best sequences among %s with up to %d simultaneous mutations ...",
				numSequences,
			...
			while (true) {
				...
				if (node.getScore() > objectiveWindowMax || (!infos.isEmpty() && node.getScore() > infos.get(0).objective.upper + objectiveWindowSize)) {
					log("\nCOMETS-Z exiting early: exhausted all conformations in energy window");
					break;
				}
				...
				if (confs.isRefinementComplete()) {
					...
					// stop COMETS-Z if we hit the desired number of sequences
		...
		log("");
		if (infos.isEmpty()) {
			log("COMETS-Z didn't find any sequences within the window that satisfy all the constraints.");
		} else {
			log("COMETS-Z found the best %d within the window that satisfy all the constraints", infos.size());
		}
