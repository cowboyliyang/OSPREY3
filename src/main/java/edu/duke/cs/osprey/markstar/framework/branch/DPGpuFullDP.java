...
/**
 * CUDA fast path for BranchMARK* full-DP: one non-leaf edge with one or more
...
final class DPGpuFullDP {
    ...
    private static void runMultiGpu(Request req, GpuExecutor[] executors, int nGpus) throws InterruptedException {
        ...
        if (req.progress) {
            ...
            System.out.println("BranchMARK*: GPU DP multi-gpu done, gpus=" + nGpus
                    + ", mStates=" + total
    ...
    private static void runOnGpuRange(Request req, GpuStream stream, long rangeStart, long rangeSize) {
        ...
        try {
            ...
            while (remaining > 0) {
                ...
                if (req.progress) {
                    double ms = (System.nanoTime() - startNanos)/1e6;
                    System.out.println("BranchMARK*: GPU DP done, mStates=" + req.mStateCount
                            + " (chunk=" + chunk + "@" + offset + ")"
                            + ", lambdaStates=" + req.totalLambdaStates
                            + ", children=" + req.numChildren
                            + ", work=" + req.work
    ...
    private static void edgeFallback(Throwable t) {
        System.out.println("BranchMARK*: GPU DP edge fallback ("
                + t.getClass().getSimpleName() + ": " + t.getMessage()
                + "); this edge on Java DP path");
        if (Boolean.parseBoolean(System.getProperty("branchmarkstar.dp.gpu.trace", "false"))) {
            t.printStackTrace(System.err);
        }
    }
    ...
    private static void markUnavailable(String reason, Throwable t) {
        ...
        if (!unavailableLogged) {
            ...
            System.out.println("BranchMARK*: GPU DP unavailable (" + reason
                    + "); falling back to Java DP path");
            if (t != null && Boolean.parseBoolean(System.getProperty("branchmarkstar.dp.gpu.trace", "false"))) {
                t.printStackTrace(System.err);
            }
        }
    }
