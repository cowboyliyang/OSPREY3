...
final class SamplingGpuPhase1 {
    ...
    enum Method {
        ...
        static Method fromProperty(String value, Method defaultValue) {
            ...
            System.err.println("BranchMARK*: Invalid GPU sampling method '" + value
    ...
    static synchronized int[] sample(Request req) {
        ...
        try {
            ...
            if (req.progress) {
                ...
                System.out.println("BranchMARK*: GPU sampling done, samples=" + grouped.numSamples
            ...
            if (consecutiveEdgeFailures >= MAX_CONSECUTIVE_EDGE_FAILURES) {
                ...
                System.out.println("BranchMARK*: GPU sampling edge fallback ("
                        + t.getClass().getSimpleName() + ": " + t.getMessage()
                        + "); this edge on Java sampling path");
                if (Boolean.parseBoolean(System.getProperty("branchmarkstar.pac.sampling.gpu.trace", "false"))) {
    ...
    private static int[] runGumbel(Request req, Gpu gpu, GroupedSamples grouped) {
        ...
        try {
            ...
            if (req.progress) {
                double ms = (System.nanoTime() - startNanos)/1e6;
                System.out.println("BranchMARK*: GPU sampling done, samples=" + numSamples
    ...
    private static void markUnavailable(String reason, Throwable t) {
        ...
        if (!unavailableLogged) {
            ...
            System.out.println("BranchMARK*: GPU sampling unavailable (" + reason
                    + "); falling back to Java sampling path");
            if (t != null && Boolean.parseBoolean(System.getProperty("branchmarkstar.pac.sampling.gpu.trace", "false"))) {
                t.printStackTrace(System.err);
            }
        }
    }
