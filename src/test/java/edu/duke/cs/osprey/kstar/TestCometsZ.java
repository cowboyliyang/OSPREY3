...
public class TestCometsZ {

	private static class Problem {

		final CometsZ mskstar;
		final ForcefieldParams ffparams;

		public Problem(CometsZ mskstar, ForcefieldParams ffparams) {
	...
	private static Problem make2RL0PPI(boolean boundedMemory) {
		...
		CometsZ.LMFE objective = new CometsZ.LMFE.Builder()
			...
			.addState(ligand, -1.0)
			.build();
		CometsZ mskstar = new CometsZ.Builder(objective)
			.setEpsilon(epsilon)
	...
	private static void check2RL0PPI(CometsZ mskstar) {

		List<CometsZ.SequenceInfo> sequences = mskstar.findBestSequences(25);
	...
	private static void check2RL0OnlyOneMutant(CometsZ mskstar) {

		List<CometsZ.SequenceInfo> sequences = mskstar.findBestSequences(1);
	...
	private static void check2RL0WithoutWildType(CometsZ mskstar) {

		List<CometsZ.SequenceInfo> sequences = mskstar.findBestSequences(2);
	...
	private static void initStates(List<CometsZ.State> states, ForcefieldParams ffparams, boolean boundedMemory) {
	...
	private static void prepStates(Problem problem, Runnable block) {
		...
		try (EnergyCalculator ecalc = new EnergyCalculator.Builder(confSpaces, problem.ffparams)
			...
			for (CometsZ.State state : problem.mskstar.states) {
	...
	private static void bruteForce(String name, Problem problem, double epsilon) {
		...
		prepStates(problem, () -> {
			...
			for (Sequence sequence : sequences) {
				...
				Map<CometsZ.State,MathTools.DoubleBounds> stateBounds = new HashMap<>();
				for (CometsZ.State state : problem.mskstar.states) {
	...
	// COMETS-Z has been fixed to work with the new ConfDB API
	...
	private static void assertSequence(List<CometsZ.SequenceInfo> sequences, String seqStr, double objectiveLower, double objectiveUpper) {

		// find the sequence
		CometsZ.SequenceInfo info = sequences.stream()
	...
	private static void assertSequenceOrder(List<CometsZ.SequenceInfo> sequences) {
