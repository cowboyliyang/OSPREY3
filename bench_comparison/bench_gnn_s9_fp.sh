...
submit_one() {
    ...
    JID=$(sbatch $SLURM --job-name=s9_fp_${did} \
        --output=$LOGDIR/gnn_s9_fp_${did}_%j.out --error=$LOGDIR/gnn_s9_fp_${did}_%j.err \
        --wrap "cd /home/users/lz280/IdeaProjects/OSPREY3 && export LD_LIBRARY_PATH=$ORT_CUDA_LIB:$CUDA_LIB:\$LD_LIBRARY_PATH && $JAVA $JARGS \
            ...
            mv $OUTDIR/results/${did}_gnn_s9.csv $OUTDIR/results/${did}_gnn_s9_fp.csv 2>/dev/null || true" \
        | awk '{print $4}')
    echo "Submitted GNN S9-FP $did ($pdb): $JID"
}
