#!/bin/bash
# ===================================================================
# MARK* (original) benchmark on dance_bench PDBs
# Usage: bash bench_markstar.sh [design_id or "all"]
# ===================================================================
set -euo pipefail
...
submit_one() {
    ...
    JID=$(sbatch $SLURM --job-name=mark_${did} \
        --output=$LOGDIR/markstar_${did}_%j.out --error=$LOGDIR/markstar_${did}_%j.err \
        ...
        | awk '{print $4}')
    echo "Submitted MARK* $did ($pdb): $JID"
}
