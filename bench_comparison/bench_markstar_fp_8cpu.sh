...
mkdir -p "$LOGDIR" "$OUTDIR/results/markstar_fp_8cpu"
...
submit_one() {
    ...
    JID=$(sbatch $SLURM --job-name=mfp_${did} \
        --output=$LOGDIR/markstar_fp_8cpu_${did}_%j.out --error=$LOGDIR/markstar_fp_8cpu_${did}_%j.err \
        --wrap "cd /home/users/lz280/IdeaProjects/OSPREY3 && export LD_LIBRARY_PATH=$ORT_CUDA_LIB:$CUDA_LIB:\$LD_LIBRARY_PATH && $JAVA $JARGS \
            ...
            -Dosprey.bench.outputDir=$OUTDIR/results/markstar_fp_8cpu \
            ...
            -Dosprey.gnn.s10.maxSubtreeFinalizeFree=9999 \
            -Dosprey.gnn.s10.maxSubtreeFinalizeCpQ=999 \
