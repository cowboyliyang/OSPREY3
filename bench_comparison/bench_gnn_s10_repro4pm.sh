...
mkdir -p "$LOGDIR" "$OUTDIR/results/gnn_s10_repro4pm"
...
submit_one() {
    ...
    JID=$(sbatch $SLURM --job-name=s10r_${did} \
        --output=$LOGDIR/gnn_s10_repro4pm_${did}_%j.out --error=$LOGDIR/gnn_s10_repro4pm_${did}_%j.err \
        --wrap "cd /home/users/lz280/IdeaProjects/OSPREY3 && export LD_LIBRARY_PATH=$ORT_CUDA_LIB:$CUDA_LIB:\$LD_LIBRARY_PATH && $JAVA $JARGS \
            ...
            -Dosprey.bench.outputDir=$OUTDIR/results/gnn_s10_repro4pm \
