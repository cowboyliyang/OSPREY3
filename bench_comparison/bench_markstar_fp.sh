#!/bin/bash
# ===================================================================
# MARK* with fullParallelFromStart=true benchmark
# Writes to <designId>_markstar_fp.csv  (post-job rename of _markstar.csv)
# Usage: bash bench_markstar_fp.sh [design_id or "all"]
# ===================================================================
set -euo pipefail
...
submit_one() {
    ...
    JID=$(sbatch $SLURM --job-name=mark_fp_${did} \
        --output=$LOGDIR/markstar_fp_${did}_%j.out --error=$LOGDIR/markstar_fp_${did}_%j.err \
        --wrap "cd /home/users/lz280/IdeaProjects/OSPREY3 && $JAVA $JARGS \
            ...
            mv $OUTDIR/results/${did}_markstar.csv $OUTDIR/results/${did}_markstar_fp.csv 2>/dev/null || true" \
        | awk '{print $4}')
    echo "Submitted MARK*-FP $did ($pdb): $JID"
}
