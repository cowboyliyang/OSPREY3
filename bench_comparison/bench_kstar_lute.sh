#!/bin/bash
# ===================================================================
# K*+LUTE baseline benchmark on dance_bench PDBs (same 38-system suite
# as bench_kstar.sh / bench_markstar.sh / bench_packstar.sh).
# Uses LUTE (Hallen 2017) as a fast learned pairwise energy correction
# in place of per-conformation CCD, then runs classic K* enumeration
# against the fitted energy (method=kstar_lute in GenericPDBBench).
# No PAC/deterministic guarantee -- a point-estimate baseline for the
# paper's Discussion comparison against the learned-correction family.
# Usage: bash bench_kstar_lute.sh [design_id or "all"]
# ===================================================================
set -euo pipefail
...
SLURM="--account=grisman --partition=compsci --cpus-per-task=16 --mem=64G --time=14-00:00:00 --mail-user=lz280@duke.edu --mail-type=END,FAIL"
...
echo "$CP" > "$LOGDIR/.classpath_bench_lute.txt"
...
submit_one() {
    ...
    JID=$(sbatch $SLURM --job-name=kstar_lute_${did} \
        --output=$LOGDIR/kstar_lute_${did}_%j.out --error=$LOGDIR/kstar_lute_${did}_%j.err \
        --wrap "$JAVA $JARGS \
            -Dosprey.bench.pdbPath=$pdbpath \
            -Dosprey.bench.mutable='$mutable' \
            -Dosprey.bench.flexible='$flexible' \
            -Dosprey.bench.method=kstar_lute \
            -Dosprey.bench.designId=$did \
            -Dosprey.bench.outputDir=$OUTDIR/results \
            -Dosprey.bench.numCPUs=16 \
            -cp \"\$(cat $LOGDIR/.classpath_bench_lute.txt)\" $MAIN 2>&1" \
        | awk '{print $4}')
    echo "Submitted K*+LUTE $did ($pdb): $JID"
}
